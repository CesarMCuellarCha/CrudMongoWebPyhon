...
#import pymongo
import pymongo
#librería para convertir string a tipos de datos ObjectId
from bson.objectid import ObjectId

@app.route("/listarProductos", methods=['GET'])
def inicio():
    try:
        #validar la varibale de sesión creada al iniciar la sesión
...
def editar(id):
    try:
        ...
        if "username" in session:
            ...
            if request.method=="GET":          
                ...
                if nombreArchivo=="":
                    ...
                    #Actualizar el producto cuando envían foto
                    resultado = productos.update_one(productoAEditar,{'$set': atributosActualizar})
                    if (resultado.modified_count==1):
                        #subir la foto
                        rutaFoto = os.path.join(app.config['UPLOAD_FOLDER'], nombreFotoServidor)
        ...
        mensaje="Ya existe producto con ese código. Modificar"
        if nombreArchivo=="":
            #consultamos el producto para obtener el atributo foto
            #y devolverla a la vista para que la puede mostrar
